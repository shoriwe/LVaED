...
//metodo que permite obtener un elemento de acuerdo al index
    ...
    public Object getIndexNext(int n) {
        if (head == null) {
            return null;
        } else {
            ListNode puntero = head.next;
            int contador = 0;
            while (contador < n && puntero.next.next != null) {
...
//organiza la lista cuando esta son numeros
    ...
    public int IndexOf(Lista a, String b) {
        int count = 0;
        for (ListNode node = a.head; node != null; node = node.next, count++) {
            if (node.equals(b)) {
                return count;
            }
        }
